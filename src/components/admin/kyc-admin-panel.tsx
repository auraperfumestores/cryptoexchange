    ...
    <div>
      ...
      <div style={{ background: T.bg, border: `1px solid ${T.border}`, borderRadius: 16, overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.2fr 1fr 1fr 1fr', padding: '11px 18px', borderBottom: `1px solid ${T.border}`, fontSize: 10.5, fontWeight: 800, color: T.dim, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
          <span>Customer</span><span>Document</span><span>Status</span><span>Submitted</span><span>Updated</span>
        </div>

        {loading ? (
          <div style={{ padding: '40px 0', textAlign: 'center', color: T.dim, fontSize: 13 }}>Loading…</div>
        ) : rows.length === 0 ? (
          <div style={{ padding: '40px 0', textAlign: 'center', color: T.dim, fontSize: 13 }}>No submissions found.</div>
        ) : (
          rows.map(row => {
            const cfg = STATUS_CFG[row.status] ?? STATUS_CFG.collecting;
            return (
              <div
                key={row._id}
                onClick={() => router.push(`/admin/kyc/${row._id}`)}
              ...
              >
                ...
                <span style={{ fontSize: 12.5, color: T.sub }}>{row.docType ? DOC_LABEL[row.docType] : '—'}</span>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 700, color: cfg.color }}>
                  <span style={{ width: 6, height: 6, borderRadius: '50%', background: cfg.color }} />
                  {cfg.label}
                  {row.resetCount > 0 && <span style={{ fontSize: 10, color: T.dim, fontWeight: 600 }}>· reset ×{row.resetCount}</span>}
                </span>
                <span style={{ fontSize: 12, color: T.dim }}>{row.submittedAt ? formatDate(row.submittedAt) : '—'}</span>
                <span style={{ fontSize: 12, color: T.dim }}>{formatDate(row.updatedAt)}</span>
              </div>
            );
          })
        )}
