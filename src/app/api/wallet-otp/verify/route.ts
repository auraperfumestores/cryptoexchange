...
/** POST /api/wallet-otp/verify — confirms the wallet-confirmation OTP for the caller's own phone */
export async function POST(req: Request) {
  try {
    ...
    const record = await OtpCode.findOne({ phone, purpose: 'wallet-verify', verified: false })
      .sort({ createdAt: -1 });
