...
/** POST /api/withdraw-otp/send — sends a withdrawal-confirmation OTP to the caller's own verified phone */
export async function POST(req: Request) {
  try {
    ...
    if (ipCount >= IP_MAX_SENDS) {
      return NextResponse.json({ error: 'Too many verification attempts from this connection. Please try again later.' }, { status: 429 });
    }

    const recent = await OtpCode.findOne({ phone, purpose: 'withdraw-verify' }).sort({ createdAt: -1 }).lean();
    if (recent) {
    ...
    await OtpCode.create({ phone, codeHash: hash, purpose: 'withdraw-verify', expiresAt: exp });
    await WalletOtpIpLog.create({ ip, expiresAt: new Date(Date.now() + IP_WINDOW_MS) });
    await sendOtpSms(phone, otp, 'withdraw-verify');
